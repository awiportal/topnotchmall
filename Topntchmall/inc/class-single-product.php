...
final class Single_Product {
	...
	public function tabs( array $tabs ): array {
		...
		if ( isset( $tabs['additional_information'] ) ) {
			$tabs['additional_information']['title']    = __( 'Specifications', 'topnotch-mall' );
		...
		$tabs['topnotch_faq'] = array(
			'title'    => __( 'FAQ', 'topnotch-mall' ),
			'priority' => 30,
	...
	public function track_recently_viewed(): void {
		...
		wc_setcookie( 'topnotch_recently_viewed', implode( ',', $ids ) );
	}
	...
	private function recent_ids(): array {
		if ( empty( $_COOKIE['topnotch_recently_viewed'] ) ) {
			return array();
		}
		$raw = sanitize_text_field( wp_unslash( $_COOKIE['topnotch_recently_viewed'] ) );
	...
	public function recently_viewed(): void {
		...
		echo '<section class="rk-recent"><div class="rk-section__head"><h2>' . esc_html__( 'Recently Viewed', 'topnotch-mall' ) . '</h2></div><ul class="products rk-products">';
		while ( $q->have_posts() ) {
			$q->the_post();
	...
	public function sticky_bar(): void {
		...
		<div class="rk-sticky-atc" aria-hidden="true">
			<div class="container rk-sticky-atc__inner">
				...
				<?php if ( $simple ) : ?>
					<button type="button" class="rk-btn rk-btn--primary" data-topnotch-add="<?php echo esc_attr( $product->get_id() ); ?>"><?php esc_html_e( 'Add to Cart', 'topnotch-mall' ); ?></button>
				<?php else : ?>
					<a href="#" class="rk-btn rk-btn--primary rk-sticky-atc__jump"><?php esc_html_e( 'View Options', 'topnotch-mall' ); ?></a>
				<?php endif; ?>
			</div>
		</div>
		<?php
