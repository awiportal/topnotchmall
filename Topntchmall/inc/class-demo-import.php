...
final class Demo_Import {
	...
	public function import_files(): array {
		return array(
			array(
				'import_file_name'       => __( 'Topnotch Mall - Full Demo', 'topnotch-mall' ),
				'local_import_file'      => TOPNOTCH_DIR . 'demo/content.xml',
				'import_preview_image_url' => TOPNOTCH_URI . 'assets/img/logo.webp',
				'import_notice'          => __( 'Installs demo pages, menus, product categories, brands and sample products. After importing, use Products > Import to load your full product CSV.', 'topnotch-mall' ),
	...
	public function page_setup( array $default ): array {
		...
		$default['page_title']  = __( 'Topnotch Mall Demo Import', 'topnotch-mall' );
		$default['menu_title']  = __( 'Import Demo Data', 'topnotch-mall' );
		$default['capability']  = 'import';
		$default['menu_slug']   = 'topnotch-demo-import';
		return $default;
	}
	...
	public function after_import(): void {
		...
		if ( ! $primary ) {
			$primary = get_term_by( 'name', 'Topnotch primary', 'nav_menu' );
		}
