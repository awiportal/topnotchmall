...
final class Ajax {
	...
	public function hooks(): void {
		add_action( 'wp_ajax_topnotch_add_to_cart', array( $this, 'add_to_cart' ) );
		add_action( 'wp_ajax_nopriv_topnotch_add_to_cart', array( $this, 'add_to_cart' ) );

		add_action( 'wp_ajax_topnotch_search', array( $this, 'live_search' ) );
		add_action( 'wp_ajax_nopriv_topnotch_search', array( $this, 'live_search' ) );
	}
	...
	public function add_to_cart(): void {
		...
		if ( ! function_exists( 'WC' ) || null === WC()->cart ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Cart unavailable.', 'topnotch-mall' ) ), 400 );
		}
		...
		if ( ! $product_id || 'product' !== get_post_type( $product_id ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Invalid product.', 'topnotch-mall' ) ), 400 );
		}

		$added = WC()->cart->add_to_cart( $product_id, $quantity );
		if ( ! $added ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Could not add to cart.', 'topnotch-mall' ) ), 400 );
		}
		...
		foreach ( array( 'product_cat', 'product_brand' ) as $tax ) {
			...
			foreach ( (array) $found as $t ) {
				$terms[] = array(
					...
					'type'  => 'product_cat' === $tax ? esc_html__( 'Category', 'topnotch-mall' ) : esc_html__( 'Brand', 'topnotch-mall' ),
				);
			}
		}
